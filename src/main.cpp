...
#if defined(NDEBUG)
# error "Kryptocoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x7fdc074c6124ec753424ec3a472e39f711cdb098be26c7995e501823749f7a5e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Kryptocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Kryptocoin Signed Message:\n";
...
{
    // Kryptocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 105000 blocks
    nSubsidy >>= (nHeight / 105000); 

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2 * 24 * 60 * 60; // Kryptocoin: 2 days
static const int64 nTargetSpacing = 2 * 60; // Kryptocoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Kryptocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Kryptocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x23c8a3535f289c9d31edde19c7c42ffd0e2a2f6efd48f0466d66fbeaf282eb5b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "December 29, 2021: Kryptocoin development on the way";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04709eb97500ecd835b43a5d6348e644f1e922a730871523f6dd5b5a6d3078683e9dc5911380b8998aefd2063ee152d4dbf8504d8168d5a6ab39b4ccc719e55c3d") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1640818036;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085759239;

        if (fTestNet)
        {
            block.nTime    = 1640818021;
            block.nNonce   = 386473621;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xb6d300e0bb6a1e594a00307cc9119d5734bb4704a1f6ba1998573cde748f7de4"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc4, 0xb2, 0xdd }; // Kryptocoin: random
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KryptocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KryptocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KryptocoinMiner(CWallet *pwallet)
{
    printf("KryptocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("kryptocoin-miner");
    ...
    try { loop {
        ...
        printf("Running KryptocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KryptocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KryptocoinMiner, pwallet));
}
